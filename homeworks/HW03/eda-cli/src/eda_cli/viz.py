...
def plot_zeros_matrix(df: pd.DataFrame, out_path: PathLike) -> Path:
    """
    Простая визуализация нулевых значений: где True=нулевое значение, False=ненулевое значение.
    """
    out_path = Path(out_path)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    if df.empty:
        ...
        ax.set_xlabel("Columns")
        ax.set_ylabel("Rows")
        ax.set_title("Zero values matrix")
        ax.set_xticks(range(df.shape[1]))
        ax.set_xticklabels(df.columns, rotation=90, fontsize=8)
        ax.set_yticks([])
